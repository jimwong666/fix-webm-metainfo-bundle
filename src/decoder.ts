...
export default class EBMLDecoder {
  ...
  private readTag(): boolean {
    ...
    const tag = tools.readVint(this._bufferChunks[0] as any, this._cursor);
    if (tag === null) {
      return false;
    }

    const buf = tools.concat(this._sliceChunks(this._cursor, this._cursor + tag.length) as any) as any;
    const tagNum = buf.reduce((o:any, v:any, i:any, arr:any) => o + v * Math.pow(16, 2 * (arr.length - 1 - i)), 0);
  ...
  private readSize(): boolean {
    ...
    const size = tools.readVint(this._bufferChunks[0] as any, this._cursor);
  ...
  private readContent(): boolean {
    ...
    const data = tools.concat(this._sliceChunks(this._cursor, this._cursor + tagObj.dataSize) as any);
